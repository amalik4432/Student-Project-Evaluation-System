...
const sourceDB = mongoose.createConnection(sourceUri, {
  serverSelectionTimeoutMS: 10000,
});
sourceDB.on("error", (error) => {
...
const updateTeachers = async () => {
  try {
      ...
      .on("change", async (change) => {
        ...
        try {
          ...
          if (change.operationType === "insert") {
            const { _id, name, empId, image, designation } =
              change.fullDocument;
    ...
    console.error(`Teacher watcher unavailable: ${error.message}`);
  }
};
async function run() {
  updateTeachers();
